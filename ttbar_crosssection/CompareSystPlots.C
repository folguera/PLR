...
void CompareSystPlots(TString syst, TString process){
  ...
  TFile * inputfile_fit = new TFile("Template_JES_JER_BTag_LES_Q2_Mpts.root");
  ...
  hist_NJetsNBjets__rare->       SetLineWidth(1.5);
  hist_NJetsNBjets__rare__minus->SetLineWidth(1.5);
  hist_NJetsNBjets__rare__plus-> SetLineWidth(1.5);
  
  
  hist_NJetsNBjets__rare->Draw("");
  ...
  qw->Draw();
  c1->SaveAs( ("NJetsNBjets__"+process+"__"+syst+".gif").Data());
}
