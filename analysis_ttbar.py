...
#print ["profile scan ", resnll['ttbar'][0].x ]

#x = n.linspace(0, 4*n.pi,101)
#y = n.cos(x)
xprofil =  array("d", resnll['ttbar'][0].x)
yprofil =  array("d", resnll['ttbar'][0].y)
...
if dosysttable:
	...
	for p in model.get_parameters([]):
		...
		#print ["syst effect of (pb) ", p, "%.2f" %(syst*ttbar_fit)]
		print [p, "%.2f" %(syst*ttbar_fit), "%.2f" %(syst*100)]
...
if dobiasscan:
	for i in range(11):
		...
		if i==0:
			...
			res_toy = pl_interval(model, 'toys:0.5', n=10100, cls = [one_sigma], signal_process_groups = signal_shapes )
			for j in range(10000):
				theBias0p5.Fill(res_toy['ttbar'][0][j])
				pull = (res_toy['ttbar'][0][j] - 0.5)/(0.5*(res_toy['ttbar'][one_sigma][j][1] -res_toy['ttbar'][one_sigma][j][0]))
				thePull0p5.Fill(pull)
			#print ["%.4f" % res_toy['ttbar'][0][0], res_toy['ttbar'][one_sigma][0][1], res_toy['ttbar'][one_sigma][0][1]]
	        if i==1:
		        res_toy = pl_interval(model, 'toys:0.6', n=10100, cls = [one_sigma], signal_process_groups = signal_shapes )
		        for j in range(10000):
		       		theBias0p6.Fill(res_toy['ttbar'][0][j])
				pull = (res_toy['ttbar'][0][j] - 0.6)/(0.5*(res_toy['ttbar'][one_sigma][j][1] -res_toy['ttbar'][one_sigma][j][0]))
				thePull0p6.Fill(pull)
	        if i==2:
		        res_toy = pl_interval(model, 'toys:0.7', n=10100, cls = [one_sigma], signal_process_groups = signal_shapes )
		        for j in range(10000):
			        theBias0p7.Fill(res_toy['ttbar'][0][j])
				pull = (res_toy['ttbar'][0][j] - 0.7)/(0.5*(res_toy['ttbar'][one_sigma][j][1] -res_toy['ttbar'][one_sigma][j][0]))
				thePull0p7.Fill(pull)
	        if i==3:
		        res_toy = pl_interval(model, 'toys:0.8', n=10100, cls = [one_sigma], signal_process_groups = signal_shapes)
		        for j in range(10000):
			        theBias0p8.Fill(res_toy['ttbar'][0][j])
				pull = (res_toy['ttbar'][0][j] - 0.8)/(0.5*(res_toy['ttbar'][one_sigma][j][1] -res_toy['ttbar'][one_sigma][j][0]))
				thePull0p8.Fill(pull)
	        if i==4:
		        res_toy = pl_interval(model, 'toys:0.9', n=10100, cls = [one_sigma], signal_process_groups = signal_shapes)
		        for j in range(10000):
			        theBias0p9.Fill(res_toy['ttbar'][0][j])
				pull = (res_toy['ttbar'][0][j] - 0.9)/(0.5*(res_toy['ttbar'][one_sigma][j][1] -res_toy['ttbar'][one_sigma][j][0]))
				thePull0p9.Fill(pull)
	        if i==5:
		        res_toy = pl_interval(model, 'toys:1.0', n=10100, cls = [one_sigma], signal_process_groups = signal_shapes )
		        for j in range(10000):
			        theBias1p0.Fill(res_toy['ttbar'][0][j])
				pull = (res_toy['ttbar'][0][j] - 1.0)/(0.5*(res_toy['ttbar'][one_sigma][j][1] -res_toy['ttbar'][one_sigma][j][0]))
				thePull1p0.Fill(pull)
	        if i==6:
		        res_toy = pl_interval(model, 'toys:1.1', n=10100, cls = [one_sigma], signal_process_groups = signal_shapes )
		        for j in range(10000):
			        theBias1p1.Fill(res_toy['ttbar'][0][j])
				pull = (res_toy['ttbar'][0][j] - 1.1)/(0.5*(res_toy['ttbar'][one_sigma][j][1] -res_toy['ttbar'][one_sigma][j][0]))
				thePull1p1.Fill(pull)
	        if i==7:
		        res_toy = pl_interval(model, 'toys:1.2', n=10100, cls = [one_sigma], signal_process_groups = signal_shapes )
		        for j in range(10000):
			        theBias1p2.Fill(res_toy['ttbar'][0][j])
				pull = (res_toy['ttbar'][0][j] - 1.2)/(0.5*(res_toy['ttbar'][one_sigma][j][1] -res_toy['ttbar'][one_sigma][j][0]))
				thePull1p2.Fill(pull)
	        if i==8:
		        res_toy = pl_interval(model, 'toys:1.3', n=10101, cls = [one_sigma], signal_process_groups = signal_shapes )
		        for j in range(10000):
			        theBias1p3.Fill(res_toy['ttbar'][0][j])
				pull = (res_toy['ttbar'][0][j] - 1.3)/(0.5*(res_toy['ttbar'][one_sigma][j][1] -res_toy['ttbar'][one_sigma][j][0]))
				thePull1p3.Fill(pull)
	        if i==9:
		        res_toy = pl_interval(model, 'toys:1.4', n=10100, cls = [one_sigma], signal_process_groups = signal_shapes)
		        for j in range(10000):
			        theBias1p4.Fill(res_toy['ttbar'][0][j])
				pull = (res_toy['ttbar'][0][j] - 1.4)/(0.5*(res_toy['ttbar'][one_sigma][j][1] -res_toy['ttbar'][one_sigma][j][0]))
				thePull1p4.Fill(pull)         
	        if i==10:
		        res_toy = pl_interval(model, 'toys:1.5', n=10101, cls = [one_sigma], signal_process_groups = signal_shapes )
		        for j in range(10000):
			        theBias1p5.Fill(res_toy['ttbar'][0][j])
				pull = (res_toy['ttbar'][0][j] - 1.5)/(0.5*(res_toy['ttbar'][one_sigma][j][1] -res_toy['ttbar'][one_sigma][j][0]))
